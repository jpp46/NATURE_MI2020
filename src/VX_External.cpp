...
void CVX_External::setDisplacement(dofComponent dof, double displacement)
{
    ...
    {
        if (dof & X_TRANSLATE)
            extTranslation.x = displacement;
        if (dof & Y_TRANSLATE)
            extTranslation.y = displacement;
        if (dof & Z_TRANSLATE)
            extTranslation.z = displacement;
        if (dof & X_ROTATE)
            extRotation.x = displacement;
        if (dof & Y_ROTATE)
            extRotation.y = displacement;
        if (dof & Z_ROTATE)
            extRotation.z = displacement;
    }
...
void CVX_External::clearDisplacement(dofComponent dof)
{
    ...
    if (dof & X_TRANSLATE)
        extTranslation.x = 0.0;
    if (dof & Y_TRANSLATE)
        extTranslation.y = 0.0;
    if (dof & Z_TRANSLATE)
        extTranslation.z = 0.0;
    if (dof & X_ROTATE)
        extRotation.x = 0.0;
    if (dof & Y_ROTATE)
        extRotation.y = 0.0;
...
void CVX_External::rotationChanged()
{
    ...
    {
        if (!_extRotationQ)
            _extRotationQ = new Quat3D<double>;
        *_extRotationQ = Quat3D<double>(extRotation);
    }
    else
    { // rotation is zero in all axes
        if (_extRotationQ)
            *_extRotationQ = Quat3D<double>();
    }
}
