...
bool CVX_LinearSolver::solve() // formulates and solves system!
{
    ...
    if (dof == 0)
    {
        errorMsg = "No free degrees of freedom found. Aborting.\n";
        return false;
    }
    ...
    phase = 11;
    pardiso(pt, &maxfct, &mnum, &mtype, &phase, &dof, &a[0], &ia[0], &ja[0], &idum, &nrhs, iparm, &msglvl, &b[0], &x[0],
            &error, dparm);
    ...
    if (error != 0)
    {
        ...
        {
        case -1:
            errorMsg = "Pardiso error: Input inconsistent\n";
        case -2:
            errorMsg = "Pardiso error: Not enough memory\n";
        case -3:
            errorMsg = "Pardiso error: Reodering Problem\n";
        case -4:
            errorMsg = "Pardiso error: Zero pivot, numerical factorization or iterative refinement problem\n";
        case -10:
            errorMsg = "No License file Pardiso.lic found.\nGo to www.pardiso-project.org and follow the instructions "
                       "to obtain a license file for your computer.\nPlace the file in the executable directory.\n";
        case -11:
            errorMsg = "License is expired\n";
        case -12:
            errorMsg = "Wrong username or hostname\n";
        default:
            errorMsg = "Pardiso Error\n";
        }
    }
...
void CVX_LinearSolver::calculateA() // calculates the big stiffness matrix!
{
    ...
    { // for each voxel...
        ...
        {                               // for each degree of freedom of the current voxel
            ...
            {
                for (int k = 0; k < 3; k++)
                    ja[jACounter++] = 6 * i2List[m] + blockOff[j][k];
            }
...
void CVX_LinearSolver::addAValue(
    int row, int column,
    float value) // after ia and ja are populated, set a value in the a matrix. Row and Column are 0-based!
{
    int curInd = ia[row];
...
void CVX_LinearSolver::consolidateA() // gets rid of all the zeros for quicker solving! (maybe...). Assumes 1-based
                                      // arrays!
{
    ...
    for (int i = 0; i < dof; i++)
    { // for each element of ia;
        ia[i + 1] -= shift;
        while (index < ia[i + 1])
        { // for everything on this row...
            while (a[index + shift] == 0)
            { // if this is an element to remove
                shift++;
                ia[i + 1]--;
            }
...
void CVX_LinearSolver::applyBX() // Assumes 0-based indices
{
    ...
    for (int i = 0; i < vCount; i++)
    {
        ...
        Vec3D<double> angle =
            (pVox->orientation().w == 1) ? Vec3D<double>(0, 0, 0) : pVox->orientation().ToRotationVector();
        ...
        for (int j = 0; j < 6; j++)
        {
            ...
            fixed[thisDof] =
                hasExternal ? pVox->external()->isFixed(dofMap[j]) : false; // note if it is presecribed/fixed
            //			fixed[thisDof] = pVox->isFixed(dofMap[j]); //note if it is presecribed/fixed
            if (!fixed[thisDof])
                b[thisDof] = (j < 3) ? force[j] : moment[j % 3]; // add in forces if it's not a fixed degree of freedom
        }
    }
    ...
    for (int thisDof = 0; thisDof < dof; thisDof++)
    {
        if (fixed[thisDof])
        {
            int dofCounter = 0; // keep track of the row we're searching when examining the column above the active
                                // diagonal element, or the column when in the final row.
            for (int k = 0; k < ia[thisDof + 1]; k++)
            {                                      // up through the end of this row
                bool thisRow = (k >= ia[thisDof]); // searching the final row (otherwise searching the column above)
                if (thisRow)
                    dofCounter = ja[k]; // keep track of the column as we step through the row.
                if (k == ia[dofCounter + 1])
                    dofCounter++; // keep track of the row as we step through columns

                if (ja[k] == thisDof || thisRow)
                {
                    b[dofCounter] -= x[thisDof] * a[k];
                    if (k != ia[thisDof])
                        aToZero.push_back(k); // if not diagonal, it will be zeroed. However, defer zeroing since some
                                              // of these values could be used twice...
                }
            }
        }
    }
    ...
    for (int i = 0; i < (int)aToZero.size(); i++)
        a[aToZero[i]] = 0; // zero out the ones we've marked accordingly
}
