...
bool CVX_MaterialLink::updateAll()
{
    ...
    // failure stress (f) is the minimum of the two failure stresses, or if both are -1.0f it should also be -1.0f to
    // denote no failure specified
    float stressFail = -1.0f, /*strainFail=-1.0f,*/ f1 = vox1Mat->sigmaFail, f2 = vox2Mat->sigmaFail;
    if (f1 == -1.0f)
        stressFail = f2; //-1.0f or vox2Mat fail
    else if (f2 == -1.0f)
        stressFail = f1; // vox1Mat fail
    else
        stressFail = f1 < f2 ? f1 : f2; // the lesser stress denotes failure

    if (vox1Mat->linear && vox2Mat->linear)
        setModelLinear(2.0f * vox1Mat->E * vox2Mat->E / (vox1Mat->E + vox2Mat->E), stressFail);
    else
    { // at least 1 bilinear or data-based, so build up data points and apply it.
        ...
        {
            float strain = FLT_MAX; // strain for the next data point is the smaller of the two possible next strain
                                    // points (but we have to make sure we don't access off the end of one of the arrays)
            if (dataIt1 < (int)vox1Mat->strainData.size())
                strain = vox1Mat->strainData[dataIt1];
            if (dataIt2 < (int)vox2Mat->strainData.size() && vox2Mat->strainData[dataIt2] < strain)
                strain = vox2Mat->strainData[dataIt2];
            else
                assert(strain != FLT_MAX); // this should never happen

            if (strain == vox1Mat->strainData[dataIt1])
                dataIt1++;
            if (strain == vox2Mat->strainData[dataIt2])
                dataIt2++;
            ...
            newStressValues.push_back(newStressValues[lastDataIndex] +
                                      thisModulus *
                                          (strain - newStrainValues[lastDataIndex])); // springs in series equation
        }

        setModel(newStrainValues.size(), &newStrainValues[0], &newStressValues[0]);

        // override failure points in case no failure was specified before (as possible in combos of linear and bilinear
        // materials) yield point is handled correctly in setModel.
