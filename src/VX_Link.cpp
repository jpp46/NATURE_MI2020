...
CVX_Link::CVX_Link(CVX_Voxel *voxel1, CVX_Voxel *voxel2, CVX_MaterialLink *material /*, linkDirection direction*/)
{
    ...
    {
        if (voxel1->indexZ() == voxel2->indexZ() + 1)
            reverseOrder = true;
        else if (voxel1->indexZ() + 1 == voxel2->indexZ())
            reverseOrder = false;
        else
            return; // non-adjacent
        axis = Z_AXIS;
    ...
    {
        if (voxel1->indexY() == voxel2->indexY() + 1)
            reverseOrder = true;
        else if (voxel1->indexY() + 1 == voxel2->indexY())
            reverseOrder = false;
        else
            return; // non-adjacent
        axis = Y_AXIS;
    ...
    {
        if (voxel1->indexX() == voxel2->indexX() + 1)
            reverseOrder = true;
        else if (voxel1->indexX() + 1 == voxel2->indexX())
            reverseOrder = false;
        else
            return; // non-adjacent
        axis = X_AXIS;
...
{
    Vec3D<double> oldPos2 = pos2, oldAngle1v = angle1v,
                  oldAngle2v = angle2v; // remember the positions/angles from last timestep to calculate velocity

    orientLink(/*restLength*/); // sets pos2, angle1, angle2

    Vec3D<double> dPos2 =
        0.5 * (pos2 - oldPos2); // deltas for local damping. velocity at center is half the total velocity
    ...
    // Beam equations. All relevant terms are here, even though some are zero for small angle and others are zero for
    // large angle (profiled as negligible performance penalty)
    forceNeg = Vec3D<double>(
        _stress * currentTransverseArea, // currentA1*pos2.x,
        b1 * pos2.y - b2 * (angle1v.z + angle2v.z),
        b1 * pos2.z +
            b2 * (angle1v.y + angle2v.y)); // Use Curstress instead of -a1*Pos2.x to account for non-linear deformation
    forcePos = -forceNeg;

    momentNeg = Vec3D<double>(a2 * (angle2v.x - angle1v.x), -b2 * pos2.z - b3 * (2 * angle1v.y + angle2v.y),
                              b2 * pos2.y - b3 * (2 * angle1v.z + angle2v.z));
    momentPos = Vec3D<double>(a2 * (angle1v.x - angle2v.x), -b2 * pos2.z - b3 * (angle1v.y + 2 * angle2v.y),
                              b2 * pos2.y - b3 * (angle1v.z + 2 * angle2v.z));

    // local damping:
    if (isLocalVelocityValid())
    { // if we don't have the basis for a good damping calculation, don't do any damping.
        float sqA1 = mat->_sqA1, sqA2xIp = mat->_sqA2xIp, sqB1 = mat->_sqB1, sqB2xFMp = mat->_sqB2xFMp,
              sqB3xIp = mat->_sqB3xIp;
        Vec3D<double> posCalc(sqA1 * dPos2.x, sqB1 * dPos2.y - sqB2xFMp * (dAngle1.z + dAngle2.z),
        ...
        momentNeg -=
            ...
            Vec3D<>(-sqA2xIp * (dAngle2.x - dAngle1.x), sqB2xFMp * dPos2.z + sqB3xIp * (2 * dAngle1.y + dAngle2.y),
                    -sqB2xFMp * dPos2.y + sqB3xIp * (2 * dAngle1.z + dAngle2.z));
        momentPos -=
            0.5 * pVPos->dampingMultiplier() *
            Vec3D<>(sqA2xIp * (dAngle2.x - dAngle1.x), sqB2xFMp * dPos2.z + sqB3xIp * (dAngle1.y + 2 * dAngle2.y),
    ...
    assert(!(forceNeg.x != forceNeg.x) || !(forceNeg.y != forceNeg.y) || !(forceNeg.z != forceNeg.z)); // assert non
                                                                                                       // QNAN
    assert(!(forcePos.x != forcePos.x) || !(forcePos.y != forcePos.y) || !(forcePos.z != forcePos.z)); // assert non
                                                                                                       // QNAN
}
...
float CVX_Link::updateStrain(float axialStrain)
{
    ...
    else
    {
        ...
        {                            // if new territory on the stress/strain curve
            ...
            if (mat->nu != 0.0f)
                strainOffset =
                    maxStrain - mat->stress(axialStrain) /
                                    (mat->_eHat * (1 - mat->nu)); // precalculate strain offset for when we back off
            else
                strainOffset = maxStrain - returnStress / mat->E; // precalculate strain offset for when we back off
        }
        else
        { // backed off a non-linear material, therefore in linear region.
            float relativeStrain = axialStrain - strainOffset; // treat the material as linear with a strain offset
                                                               // according to the maximum plastic deformation
...
float CVX_Link::strainEnergy() const
{
    return forceNeg.x * forceNeg.x / (2.0f * mat->_a1) +  // Tensile strain
           ...
           (momentNeg.z * momentNeg.z - momentNeg.z * momentPos.z + momentPos.z * momentPos.z) /
               (3.0 * mat->_b3) + // Bending Z
           (momentNeg.y * momentNeg.y - momentNeg.y * momentPos.y + momentPos.y * momentPos.y) /
...
float CVX_Link::a1() const
{
    return mat->_a1;
}
float CVX_Link::a2() const
{
    return mat->_a2;
}
float CVX_Link::b1() const
{
    return mat->_b1;
}
float CVX_Link::b2() const
{
    return mat->_b2;
}
float CVX_Link::b3() const
{
    return mat->_b3;
}
