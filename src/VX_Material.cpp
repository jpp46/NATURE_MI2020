...
void CVX_Material::writeJSON(rapidjson::PrettyWriter<rapidjson::StringBuffer> &w)
{
    ...
    else
    {
        w.Key("strainData");
        w.StartArray();
        for (int i = 0; i < (int)strainData.size(); i++)
            w.Double((double)strainData[i]);
        w.EndArray();

        w.Key("stressData");
        w.StartArray();
        for (int i = 0; i < (int)stressData.size(); i++)
            w.Double((double)stressData[i]);
        w.EndArray();
    }
    ...
    if (muStatic != 0)
    {
        w.Key("staticFriction");
        w.Double(muStatic);
    }
    if (muKinetic != 0)
    {
        w.Key("kineticFriction");
        w.Double(muKinetic);
    }
    if (zetaInternal != 1)
    {
        w.Key("internalDamping");
        w.Double(zetaInternal);
    }
    if (zetaGlobal != 0)
    {
        w.Key("globalDamping");
    ...
    if (zetaCollision != 1)
    {
        w.Key("collisionDamping");
        w.Double(zetaCollision);
    }
...
bool CVX_Material::readJSON(rapidjson::Value &m)
{
    ...
    else if (m.HasMember("strainData") && m["strainData"].IsArray() && m.HasMember("stressData") &&
             m["stressData"].IsArray() && m["strainData"].Size() == m["stressData"].Size())
    {
    ...
    if (m.HasMember("density") && m["density"].IsDouble())
        rho = m["density"].GetDouble();
    if (m.HasMember("name") && m["name"].IsString())
        myName = std::string(m["name"].GetString());
    if (m.HasMember("red") && m["red"].IsInt())
        r = m["red"].GetInt();
    if (m.HasMember("green") && m["green"].IsInt())
        g = m["green"].GetInt();
    if (m.HasMember("blue") && m["blue"].IsInt())
        b = m["blue"].GetInt();
    if (m.HasMember("alpha") && m["alpha"].IsInt())
        a = m["alpha"].GetInt();
    if (m.HasMember("poissonsRatio") && m["poissonsRatio"].IsDouble())
        nu = m["poissonsRatio"].GetDouble();
    if (m.HasMember("CTE") && m["CTE"].IsDouble())
        alphaCTE = m["CTE"].GetDouble();
    if (m.HasMember("staticFriction") && m["staticFriction"].IsDouble())
        muStatic = m["staticFriction"].GetDouble();
    if (m.HasMember("kineticFriction") && m["kineticFriction"].IsDouble())
        muKinetic = m["kineticFriction"].GetDouble();
    if (m.HasMember("internalDamping") && m["internalDamping"].IsDouble())
        zetaInternal = m["internalDamping"].GetDouble();
    if (m.HasMember("globalDamping") && m["globalDamping"].IsDouble())
        zetaGlobal = m["globalDamping"].GetDouble();
    if (m.HasMember("collisionDamping") && m["collisionDamping"].IsDouble())
        zetaCollision = m["collisionDamping"].GetDouble();
    if (m.HasMember("externalScaleFactor") && m["externalScaleFactor"].IsArray() &&
        m["externalScaleFactor"].Size() == 3)
    {
        for (int i = 0; i < 3; i++)
            extScale[i] = m["externalScaleFactor"][i].GetDouble();
    }
...
float CVX_Material::stress(float strain, float transverseStrainSum, bool forceLinear)
{
    ...
    { // go through each segment in the material model (skipping the first segment because it has already been handled.
        if (strain <= strainData[i] || i == DataCount - 1)
        { // if in the segment ending with this point (or if this is the last point extrapolate out)
            ...
            { // accounting for volumetric effects
                ...
                float effectiveStrain =
                    basicStress / modulus; // this is the strain at which a simple linear stress strain line would hit
                                           // this point at the definied modulus
...
{
    if (stress <= stressData[1] || linear)
        return stress /
               E; // for compression/first segment and linear materials (forced or otherwise), simple calculation

    int DataCount = modelDataPoints();
    for (int i = 2; i < DataCount; i++)
    { // go through each segment in the material model (skipping the first segment because it has already been handled.
        if (stress <= stressData[i] || i == DataCount - 1)
        { // if in the segment ending with this point (or if this is the last point extrapolate out)
...
float CVX_Material::modulus(float strain)
{
    ...
    { // go through each segment in the material model (skipping the first segment because it has already been handled.
        if (strain <= strainData[i] || i == DataCount - 1)
            return (stressData[i] - stressData[i - 1]) /
                   (strainData[i] - strainData[i - 1]); // if in the segment ending with this point
    }
...
The last data point is assumed to represent failure of the material. The 0.2% offset method is used to calculate the
yield point.
...
bool CVX_Material::setModel(int dataPointCount, float *pStrainValues, float *pStressValues)
{
    ...
    {                    // if first data point is 0,0, ignore it
    ...
    assert((int)tmpStrainData.size() == dataPointCount + 1 &&
           (int)tmpStressData.size() == dataPointCount + 1); // sizes match up? (+1 to include the zero point)
    ...
    { //.2% (0.002) strain offset to find a good yield point...
        setYieldFromData();
    }
...
Yield stress is interpreted as identical to failure stress. If failure stress is not specified an arbitrary data point
consistent with the specified Young's modulus is added to the model.
*/
